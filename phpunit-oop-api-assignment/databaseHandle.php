...
class DatabaseConnection{
	...
	public function updateProduct($productId,$newProductName,$productDescription,$productPrice,$productDiscount,$productCategoryId){
		try{
			$result=0;
			$updateQry=$this->connection->prepare("update products set name=?,description=?,price=?,discount=?,category_id=? where id=?");
			$result=$updateQry->execute(array($newProductName,$productDescription,$productPrice,$productDiscount,$productCategoryId,$productId));
			if(1==$result){
